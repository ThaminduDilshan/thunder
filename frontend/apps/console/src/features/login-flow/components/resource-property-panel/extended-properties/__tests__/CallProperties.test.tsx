...
describe('CallProperties', () => {
  ...
  it('disables the dropdown while flows are loading', () => {
    ...
    // The MUI Select root has aria-disabled when disabled
    const button = select.querySelector('[role="combobox"]');
    expect(button).toHaveAttribute('aria-disabled', 'true');
  });

  it('lists flows from the API and excludes the current resource', () => {
    mockUseGetFlows.mockReturnValue({
      data: {
        flows: [
          {id: 'flow-a', name: 'Flow A', flowType: 'AUTHENTICATION'},
          {id: 'flow-b', name: 'Flow B', flowType: 'REGISTRATION'},
          {id: 'call-step-1', name: 'Self', flowType: 'AUTHENTICATION'}, // same as resource id — should be filtered out
        ],
      },
      isLoading: false,
      error: null,
  ...
  it('writes the chosen flow id back to data.flow', () => {
    mockUseGetFlows.mockReturnValue({
      data: {
        flows: [{id: 'flow-a', name: 'Flow A', flowType: 'AUTHENTICATION'}],
      },
      isLoading: false,
      error: null,
    });
